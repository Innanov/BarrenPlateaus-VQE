...
@testset "VQE Methods Tests" begin
    ...
    result_sea = run_vqe(
        vqe_sea, H, random_initial_parameters(vqe_sea.n_parameters), 5, verbose=false
    )
...
@testset "Molecular Analyzer Tests" begin
    ...
    results = run_complete_analysis(
        analyzer, num_iters=5, methods=["standard"], verbose=false
    )
...
@testset "Integration Test" begin
    ...
    # 3. Run analysis
    results = run_complete_analysis(
        analyzer, num_iters=10, methods=["standard", "local_global"], verbose=false
    )
