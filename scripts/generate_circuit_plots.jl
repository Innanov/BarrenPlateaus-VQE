...
"""
function generate_circuit_plots_for_molecule(
    ...
    try
        ...
        circuit_files = BarrenPlateausVQE.CircuitVisualization.visualize_all_ansatz_circuits(
            analyzer; output_dir=molecule_output_dir, file_format=format
        )
        ...
        if include_analysis
            ...
            analysis_files = create_additional_circuit_analysis(
                analyzer, molecule_output_dir, format, verbose
            )
            append!(generated_files, analysis_files)
        end

        # Step 5: Create summary for this molecule
        create_molecule_summary(
            molecule_name, analyzer, molecule_output_dir, generated_files, verbose
        )
...
function create_analyzer_with_full_analysis(molecule_name::String, verbose::Bool)
    try
        ...
        if verbose
            ...
            println(
                "    • Exact energy: $(round(analyzer.molecular_system.exact_energy, digits=6)) Ha",
            )
        end
        ...
        try
            ...
            if verbose
                ...
                if isa(e1, MethodError) && string(e1.f) == "*"
                    println(
                        "    🔍 Detected string multiplication error - likely separator line issue",
                    )
                end
            end
        ...
        if analysis_success && verbose
            ...
            for (method_name, data) in analyzer.results
                ...
                if !get(method_result, "fallback", false)
                    ...
                    if method_name == "PretrainedVQE" && haskey(method_result, "full")
                        ansatz = get(
                            get(method_result["full"], "ansatz_info", Dict()),
                            "ansatz",
                            nothing,
                        )
                    elseif method_name == "LocalGlobalVQE" && haskey(method_result, "out")
                        ansatz = get(
                            get(method_result["out"], "ansatz_info", Dict()),
                            "ansatz",
                            nothing,
                        )
                    else
                        ansatz = get(
                            get(method_result, "ansatz_info", Dict()), "ansatz", nothing
                        )
                    end
            ...
            println(
                "    ✅ Successfully created circuits for $circuit_count/$(length(analyzer.results)) methods",
            )
        end
...
function run_individual_methods_safely(analyzer, verbose::Bool)
    ...
    for (method_name, constructor) in methods
        ...
        try
            ...
            analyzer.results[method_name] = Dict(
                "method_result" => result, "fallback" => false
            )
            ...
            # Create failed entry
            analyzer.results[method_name] = Dict(
                "method_result" => Dict("fallback" => true, "error" => string(e))
            )
        end
    end
    ...
    if verbose
        successful_count = count(
            method ->
                !get(
                    get(analyzer.results[method], "method_result", Dict()),
                    "fallback",
                    false,
                ),
        ...
        println(
            "      ✅ Individual execution: $successful_count/$(length(methods)) methods succeeded",
        )
    end
end
...
"""
function create_additional_circuit_analysis(
    analyzer, output_dir::String, format::String, verbose::Bool
)
    ...
    try
        ...
        circuit_collection = BarrenPlateausVQE.CircuitVisualization.analyze_circuit_architectures(
            analyzer
        )
        ...
        comparison_grid = BarrenPlateausVQE.CircuitVisualization.create_ansatz_comparison_grid(
            circuit_collection; save_path=grid_path
        )
        ...
        complexity_files = create_circuit_complexity_plots(
            circuit_collection, output_dir, format
        )
...
"""
function create_circuit_complexity_plots(
    circuit_collection, output_dir::String, format::String
)
    ...
    try
        ...
        for i in 1:length(valid_circuits)
            Plots.annotate!(
                p2,
                parameters[i],
                complexities[i] + 0.02 * maximum(complexities),
        ...
        for i in 1:length(valid_circuits)
            Plots.annotate!(
                p3,
                depths[i],
                gate_counts[i] + 0.02 * maximum(gate_counts),
                Plots.text(split(method_names[i])[1], 8, :center),
            )
        end
...
function create_markdown_table(df::DataFrame, file_path::String, molecule_name::String)
    open(file_path, "w") do f
        ...
        write(
            f,
            "| Method | Type | Qubits | Parameters | Depth | Gates | Complexity | Param/Qubit | Gates/Qubit |\n",
        )
        write(
            f,
            "|--------|------|--------|------------|-------|-------|------------|-------------|-------------|\n",
        ...
        for row in eachrow(df)
            write(
                f,
                "| $(row.Method) | $(row.Method_Type) | $(row.Qubits) | $(row.Parameters) | $(row.Circuit_Depth) | $(row.Total_Gates) | $(round(row.Complexity_Score, digits=2)) | $(row.Param_per_Qubit) | $(row.Gates_per_Qubit) |\n",
            )
        end
        ...
        write(
            f,
            "- **Simplest Circuit**: $(df.Method[simplest_idx]) (Complexity: $(round(df.Complexity_Score[simplest_idx], digits=2)))\n",
        )
        write(
            f,
            "- **Most Complex Circuit**: $(df.Method[most_complex_idx]) (Complexity: $(round(df.Complexity_Score[most_complex_idx], digits=2)))\n",
        )
        write(
            f,
            "- **Most Parameters**: $(df.Method[most_params_idx]) ($(df.Parameters[most_params_idx]) parameters)\n",
        )
        write(
            f,
            "- **Deepest Circuit**: $(df.Method[deepest_idx]) (Depth: $(df.Circuit_Depth[deepest_idx]))\n",
        )
        ...
        write(
            f, "- **Average Complexity**: $(round(mean(df.Complexity_Score), digits=2))\n"
        )
        write(
            f,
            "- **Parameter Density Range**: $(minimum(df.Param_per_Qubit)) - $(maximum(df.Param_per_Qubit)) params/qubit\n",
        )
    end
end
...
"""
function create_molecule_summary(
    molecule_name::String,
    analyzer,
    output_dir::String,
    files::Vector{String},
    ...
    try
        open(summary_path, "w") do f
            ...
            write(
                f,
                "- **Exact Ground State Energy**: $(round(analyzer.molecular_system.exact_energy, digits=6)) Ha\n",
            )
            ...
            analysis_files = filter(
                f ->
                    contains(f, "analysis") ||
                    contains(f, "comparison") ||
                    contains(f, "complexity"),
                files,
            )
            ...
            write(
                f,
                "- Circuit plots show the quantum gate structure of each VQE method's ansatz\n",
            )
            write(
                f, "- Comparison grid allows visual comparison of circuit architectures\n"
            )
            write(
                f,
                "- Complexity analysis reveals trade-offs between expressibility and circuit depth\n",
            )
...
"""
function create_master_summary(
    output_dir::String, all_files::Dict{String,Vector{String}}, verbose::Bool
)
    ...
    try
        open(summary_path, "w") do f
            ...
            for (molecule, files) in sort(collect(all_files))
                ...
                write(
                    f,
                    "- Directory: [`$(lowercase(molecule))/`]($(lowercase(molecule))/)\n\n",
                )
            end
            ...
            write(
                f,
                "1. **StandardVQE** - Hardware-efficient ansatz with parameterized gates\n",
            )
...
function main()
    ...
    if total_files > 0
        total_size = sum(
            calculate_total_size(files) for files in values(all_generated_files)
        )
