...
function compute_gradient_finite_diff_robust(
    hamiltonian, ansatz, params, n_qubits; epsilon=1e-4
)
...
function compute_gradient_variance_statistics(
    hamiltonian, ansatz, n_params, n_qubits, n_samples, epsilon, verbose=false
)
    ...
    for sample in 1:n_samples
        try
            ...
            gradient = compute_gradient_finite_diff_robust(
                hamiltonian, ansatz, params, n_qubits; epsilon=epsilon
            )
        ...
        try
            ...
            elseif method_name == "pretrained"
                try
                    ...
                    test_energy = energy_evaluation(
                        hamiltonian, ansatz, test_params, n_qubits
                    )
            ...
            grad_stats = try
                ...
                if !isnan(computed_stats.mean_var) &&
            ...
            if verbose
                println(
                    "    Mean gradient variance: $(round(grad_stats.mean_var, digits=8))"
                )
...
function create_visualizations(all_results, output_dir, molecule, verbose=false)
    ...
    try
        ...
        p_var = plot(;
            title="Gradient Variance vs Circuit Depth\n$molecule | equilibrium | sto-3g",
        ...
        p_norm = plot(;
            title="Gradient Norms vs Circuit Depth\n$molecule | equilibrium | sto-3g",
            xlabel="Number of Layers",
        ...
        for (method, result) in all_results
            if !isempty(result["layer_counts"])
                ...
                if !isempty(result["std_grad_vars"]) &&
                    !any(isnan.(result["std_grad_vars"]))
                    scatter!(
                        p_var,
                        result["layer_counts"],
                        result["mean_grad_vars"];
                ...
                if !isempty(result["std_grad_norms"]) &&
                    !any(isnan.(result["std_grad_norms"]))
                    scatter!(
                        p_norm,
                        result["layer_counts"],
                        result["mean_grad_norms"];
...
function main()
    ...
    output_dir, run_id = setup_output_directory(
        args["output-dir"], args["molecule"], args["max-layers"]
    )
    ...
    try
        ...
        for method in methods_to_test
            ...
            if !isempty(result["mean_grad_vars"]) && length(result["mean_grad_vars"]) > 1
                ...
                if args["verbose"]
                    println("\n  📈 Summary for $method:")
                    println(
                        "    Initial variance ($(args["min-layers"]) layer): $(round(initial_var, digits=8))",
                    )
                    println(
                        "    Final variance ($(args["max-layers"]) layers): $(round(final_var, digits=8))",
                    )
        ...
        visualization_files = create_visualizations(
            all_results, output_dir, args["molecule"], args["verbose"]
        )
        ...
        data_files = export_data_and_summary(
            all_results, output_dir, args["molecule"], args, args["verbose"]
        )
