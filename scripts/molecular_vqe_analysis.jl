...
function generate_analysis_summary(analyzer, results, performance_summary, output_dir::String, molecule_name::String)
...
function main()
    ...
    output_dir, run_id = setup_output_directory(args["output-dir"], args["molecule"], args["layers"])
    ...
    try
        ...
        println("  Exact ground state energy: $(round(molecular_system.exact_energy, digits=8))")
        ...
        try
            ...
            try
                ...
                landscape_3d_files = plot_all_methods_loss_landscape_3d(analyzer,
                                                                       save_path=joinpath(output_dir, "loss_landscape_3d_dummy.pdf"))
                ...
                if !landscape_files_found
                    println("  ⚠️  No individual 3D landscape files found, checking for combined file...")
            ...
            try
                ...
                if !contour_files_found
                    println("  ⚠️  No individual contour landscape files found, checking for combined file...")
                    # Check if a combined file was created
        ...
        summary = generate_analysis_summary(analyzer, results, performance_summary, output_dir, args["molecule"])
        ...
        if !isempty(performance_summary)
            ...
            for (i, perf) in enumerate(performance_summary)
                ...
                println("  $(perf.method): Error $(round(perf.energy_error, digits=8))$indicator_str")
            end
        end
