...
"""
function cnot_layer(n_qubits::Int, n_cnot::Union{Int, String, Vector{Vector{Int}}}="Full_connect")
...
"""
function run_vqe(vqe::SEAVQE, hamiltonian::AbstractBlock, 
                initial_guess::Vector{Float64}, num_iters::Int;
                optimizer::Symbol=:SPSA,
                verbose::Bool=false,
                callback=nothing)
    ...
    if length(initial_guess) != vqe.n_parameters
        throw(ArgumentError("Initial guess length ($(length(initial_guess))) must match number of parameters ($(vqe.n_parameters))"))
    end
...
"""
function run_sea_vqe(hamiltonian::AbstractBlock, n_qubits::Int, 
                    initial_guess::Vector{Float64}, num_iters::Int;
                    depth::Vector{Int}=Int[], kwargs...)
