...
"""
function make_adiabatic_cost_function(H_local::AbstractBlock, H_global::AbstractBlock,
                                     ansatz::AbstractBlock, n_qubits::Int, s::Float64)
...
"""
function run_vqe(vqe::AdiabaticVQE, hamiltonian_in::AbstractBlock, hamiltonian_out::AbstractBlock,
                initial_guess::Vector{Float64}, num_iters::Int;
                transition_lims::Tuple{Float64,Float64}=(0.0, 1.0),
                optimizer::Symbol=:SPSA,
                verbose::Bool=false,
    ...
    # Adiabatic schedule
    s_values = collect(range(transition_lims[1], transition_lims[2], length=vqe.adiabatic_steps))
    ...
    if verbose
        println("Adiabatic VQE: $(vqe.adiabatic_steps) steps, $iterations_per_step iterations per step")
    end
    
    for (step, s) in enumerate(s_values)
        if verbose
            println("Adiabatic step $step/$(vqe.adiabatic_steps): s = $(round(s, digits=3))")
        end
        
        # Create cost function for this interpolation parameter
        cost_function = make_adiabatic_cost_function(hamiltonian_in, hamiltonian_out, 
                                                    vqe.ansatz, vqe.n_qubits, s)
        ...
        step_result = run_vqe_with_cost_function(step_vqe, cost_function, current_params, 
                                                iterations_per_step; optimizer=optimizer, verbose=false)
    ...
    final_energy = energy_evaluation(hamiltonian_out, vqe.ansatz, current_params, vqe.n_qubits)
...
"""
function run_vqe(vqe::AdiabaticVQE, hamiltonian::AbstractBlock, 
                initial_guess::Vector{Float64}, num_iters::Int;
                kwargs...)
...
"""
function run_vqe_with_cost_function(vqe::StandardVQE, cost_function, 
                                   initial_guess::Vector{Float64}, num_iters::Int;
                                   optimizer::Symbol=:SPSA, verbose::Bool=false)
...
"""
function custom_adiabatic_schedule(schedule_function::Function, 
                                  transition_lims::Tuple{Float64,Float64}=(0.0, 1.0))
...
"""
function run_adiabatic_vqe(hamiltonian_local::AbstractBlock, hamiltonian_global::AbstractBlock,
                          n_qubits::Int, initial_guess::Vector{Float64}, num_iters::Int;
                          n_layers::Int=1, kwargs...)
    ...
    return run_vqe(vqe, hamiltonian_local, hamiltonian_global, initial_guess, num_iters; kwargs...)
end
