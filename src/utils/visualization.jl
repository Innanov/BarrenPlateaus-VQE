...
"""
function compute_loss_landscape_2d(
    cost_function,
    ...
    param_i_range = range(
        center_params[i] - param_range, center_params[i] + param_range; length=resolution
    )
    param_j_range = range(
        center_params[j] - param_range, center_params[j] + param_range; length=resolution
    )
...
```
"""
function compute_trajectory_bounds(
    parameter_history::Vector{Vector{Float64}}, param_indices::Tuple{Int,Int}
)
...
function compute_optimization_trajectory_2d(
    parameter_history::Vector{Vector{Float64}}; param_indices::Tuple{Int,Int}=(1, 2)
)
    i, j = param_indices
...
function create_text_placeholder(message::String)
    return Dict(
        "type" => "text_placeholder", "message" => message, "display_text" => "📊 $message"
    )
end
...
"""
function plot_all_methods_energy_convergence(
    analyzer::MolecularVQEAnalyzer; save_path::Union{String,Nothing}=nothing
)
...
"""
function plot_all_methods_loss_landscape_3d(
    ...
    for (i, (method_name, data)) in enumerate(valid_methods)
        try
            ...
            println(
                "  📊 $method_name: Using parameter range ±$(round(actual_range, digits=3))"
            )
            ...
            p_individual = PLOTS_MODULE.surface(
                collect(param_i_range),
                collect(param_j_range),
                landscape';
            ...
            if !isempty(param_history)
                ...
                if length(trajectory_i) > 1
                    ...
                    if !isnan(trajectory_energies[1])
                        PLOTS_MODULE.scatter!(
                            p_individual,
                            [trajectory_i[1]],
                            [trajectory_j[1]],
                            [trajectory_energies[1]];
                            markersize=15,
                            color=:lime,
                            label="Start",
                            markerstrokewidth=3,
                    ...
                    println(
                        "    Trajectory spans: θ$(param_indices[1])=±$(round(traj_range_i/2, digits=3)), θ$(param_indices[2])=±$(round(traj_range_j/2, digits=3))",
                    )
                end
            end
...
"""
function plot_all_methods_loss_landscape_contour(
    analyzer::MolecularVQEAnalyzer;
    param_indices::Tuple{Int,Int}=(1, 2),
    base_param_range::Float64=0.4,
    resolution::Int=80,
    ...
    for (i, (method_name, data)) in enumerate(valid_methods)
        try
            ...
            method_title = replace(method_name, "_" => " ")
            p_individual = PLOTS_MODULE.contour(
                collect(param_i_range),
                collect(param_j_range),
                landscape';
                title="$method_title VQE Loss Landscape\n$(subtitle)",
            ...
            if save_path !== nothing
                ...
                save_dir = dirname(save_path)
                clean_name = replace(method_name, " " => "_", "/" => "_")
                individual_save_path = joinpath(
                    save_dir, "landscape_contour_$(clean_name).pdf"
                )
...
"""
function create_performance_table_all_methods(
    analyzer::MolecularVQEAnalyzer;
    save_csv::Union{String,Nothing}=nothing,
    save_latex::Union{String,Nothing}=nothing,
)
    ...
    println("="^90)
    println(
        "System: $(analyzer.molecular_system.name) ($(analyzer.molecular_system.geometry_type))",
    )
    ...
    header = safe_sprintf(
        "%-20s %-12s %-12s %-12s %-15s %-12s %-10s %-10s",
        "Method",
        "Final Energy",
        "Energy Error",
        "Fidelity",
    ...
    for i in 1:nrow(df)
        if isnan(df.Energy_Error[i])
            # Fallback method
            row = @sprintf(
                "%-20s %-12s %-12s %-12s %-15s %-12s %-10.1f %-10s",
                df.Method[i],
                "FAILED",
                "FAILED",
                "FAILED",
            ...
            # Valid method
            row = @sprintf(
                "%-20s %-12.6f %-12.2e %-12.3f %-15.2e %-12.2e %-10.1f %-10s",
                df.Method[i],
                df.Final_Energy[i],
                df.Energy_Error[i],
                df.State_Fidelity[i],
...
"""
function create_latex_table_all_methods(
    df::DataFrame, analyzer::MolecularVQEAnalyzer, save_path::String
)
    latex_content = """
\\begin{table}[h!]
\\centering
...
"""
function plot_loss_landscape_3d(
    cost_function,
    center_params::Vector{Float64};
    param_indices::Tuple{Int,Int}=(1, 2),
    param_range::Float64=0.5,
...
"""
function plot_loss_landscape_contour(
    cost_function,
    center_params::Vector{Float64};
    param_indices::Tuple{Int,Int}=(1, 2),
    param_range::Float64=0.5,
    ...
    if isempty(analyzer.results)
        ...
        if !check_plotting_available()
            return create_text_placeholder(
                "System ready: $(analyzer.molecular_system.name), $(analyzer.n_qubits) qubits",
            )
        end
        ...
        try
            ...
            PLOTS_MODULE.annotate!(
                p,
                5,
                8,
                PLOTS_MODULE.text("System: $(analyzer.molecular_system.name)", :center, 16),
            )
            PLOTS_MODULE.annotate!(
                p, 5, 7, PLOTS_MODULE.text("Qubits: $(analyzer.n_qubits)", :center, 14)
            )
            PLOTS_MODULE.annotate!(
                p, 5, 6, PLOTS_MODULE.text("Layers: $(analyzer.n_layers)", :center, 14)
            )
            PLOTS_MODULE.annotate!(
                p, 5, 4, PLOTS_MODULE.text("Ready for Analysis!", :center, 18, :green)
            )
            PLOTS_MODULE.annotate!(
                p,
                5,
                2,
                PLOTS_MODULE.text(
                    "Run: results = run_complete_analysis(analyzer)", :center, 12, :gray
                ),
            )
            ...
            return create_text_placeholder(
                "Ready: $(analyzer.molecular_system.name), $(analyzer.n_qubits) qubits"
            )
        end
    end
...
function create_placeholder_plot(title::String, message::String)
    ...
    try
        ...
        PLOTS_MODULE.annotate!(
            ...
            0.3,
            PLOTS_MODULE.text(
                "Run analyzer.run_complete_analysis() first", :center, 10, :gray
            ),
        )
...
function create_comprehensive_visualization(
    ...
    try
        conv_plot = plot_all_methods_energy_convergence(
            analyzer; save_path=joinpath(output_dir, "energy_convergence_all_methods.pdf")
        )
    ...
    try
        df = create_performance_table_all_methods(
            analyzer; save_csv=joinpath(output_dir, "performance_table_all_methods.csv")
        )
        if df !== nothing
...
function save_analysis_summary(analyzer::MolecularVQEAnalyzer, output_dir::String)
    ...
    open(summary_path, "w") do f
        ...
        if isempty(analyzer.results)
            write(
                f,
                "No analysis results available. Run `run_complete_analysis(analyzer)` first.\n",
            )
        else
            for (method_name, data) in analyzer.results
                write(f, "### $method_name\n")
                ...
                if get(data["method_result"], "fallback", false)
                    ...
                    write(
                        f,
                        "- **Execution Time**: $(round(data["execution_time"], digits=2))s\n\n",
                    )
                else
                    write(
                        f,
                        "- **Final Energy**: $(data["method_result"]["vqe_result"].final_energy)\n",
                    )
                    write(
                        f,
                        "- **Energy Error**: $(data["performance_metrics"]["final_energy_error"])\n",
                    )
                    write(
                        f,
                        "- **Gradient Variance**: $(data["bp_diagnostics"].gradient_variance)\n",
                    )
                    write(
                        f,
                        "- **State Fidelity**: $(data["performance_metrics"]["state_fidelity"])\n",
                    )
                    write(
                        f,
                        "- **Execution Time**: $(round(data["execution_time"], digits=2))s\n",
...
export plot_all_methods_energy_convergence,
    plot_all_methods_loss_landscape_3d, plot_all_methods_loss_landscape_contour
