...
struct VQEConfig
    ...
    function VQEConfig(;
        ...
        new(max_iterations, convergence_tolerance, optimizer_method, 
            random_seed, gradient_method, shot_noise, n_shots, verbose)
    end
end
...
"""
function run_vqe(method::StandardVQE, hamiltonian::AbstractBlock,
                initial_params::Union{Nothing, Vector{Float64}}=nothing)
    
    Random.seed!(method.config.random_seed)
    
    if initial_params === nothing
        initial_params = generate_initial_parameters(method.ansatz; 
    ...
    try
        if method.config.optimizer_method == :BFGS
            result = optimize(cost_function, initial_params, BFGS(),
                            Optim.Options(
        ...
        elseif method.config.optimizer_method == :NelderMead
            result = optimize(cost_function, initial_params, NelderMead(),
                            Optim.Options(
                                iterations=method.config.max_iterations,
                                callback=callback
                            ))
        elseif method.config.optimizer_method == :GradientDescent
            result = optimize(cost_function, initial_params, 
        ...
        final_params = length(parameter_history) > 0 ? parameter_history[end] : initial_params
        final_energy = length(energy_history) > 0 ? energy_history[end] : cost_function(initial_params)
        converged = false
    end
...
struct LocalGlobalVQE <: AbstractVQEMethod
    ...
    function LocalGlobalVQE(ansatz::AbstractAnsatz, config::VQEConfig;
                           local_iterations::Int=-1, global_iterations::Int=-1)
...
"""
function run_vqe(method::LocalGlobalVQE, hamiltonian::AbstractBlock,
                initial_params::Union{Nothing, Vector{Float64}}=nothing)
    ...
    if method.config.verbose
        println("Phase 1: Local Hamiltonian optimization ($(method.local_iterations) iterations)")
    end
    ...
    if method.config.verbose
        println("Phase 2: Global Hamiltonian optimization ($(method.global_iterations) iterations)")
    end
    ...
    total_parameter_history = vcat(local_result.parameter_history, global_result.parameter_history)
...
struct AdiabaticVQE <: AbstractVQEMethod
    ...
    function AdiabaticVQE(ansatz::AbstractAnsatz, config::VQEConfig;
...
"""
function interpolate_hamiltonian(H_local::AbstractBlock, H_global::AbstractBlock, s::Float64)
    if !(0.0 <= s <= 1.0)
...
"""
function run_vqe(method::AdiabaticVQE, hamiltonian::AbstractBlock,
                initial_params::Union{Nothing, Vector{Float64}}=nothing)
    ...
    if method.config.verbose
        println("Adiabatic VQE: $(method.adiabatic_steps) steps, $iterations_per_step iterations per step")
    end
    
    for (step, s) in enumerate(s_values)
        if method.config.verbose
            println("Adiabatic step $step/$(method.adiabatic_steps): s = $(round(s, digits=3))")
        end
...
"""
function run_vqe(method::SEAVQE, hamiltonian::AbstractBlock,
                initial_params::Union{Nothing, Vector{Float64}}=nothing)
...
struct PretrainedVQE <: AbstractVQEMethod
    ...
    function PretrainedVQE(mps_ansatz::MPSAnsatz, full_ansatz::AbstractAnsatz, 
                          config::VQEConfig; pretraining_iterations::Int=-1)
...
"""
function transfer_mps_parameters(mps_params::Vector{Float64}, 
                                mps_ansatz::MPSAnsatz, 
                                full_ansatz::AbstractAnsatz)
...
"""
function run_vqe(method::PretrainedVQE, hamiltonian::AbstractBlock,
                initial_params::Union{Nothing, Vector{Float64}}=nothing)
    ...
    if initial_params === nothing
        mps_initial_params = generate_initial_parameters(method.mps_ansatz; 
                                                        random_seed=method.config.random_seed)
    else
    ...
    transferred_params = transfer_mps_parameters(mps_result.final_parameters,
                                               method.mps_ansatz,
                                               method.full_ansatz)
    ...
    total_parameter_history = vcat(mps_result.parameter_history, full_result.parameter_history)
...
"""
function create_vqe_method(method_name::String, ansatz::AbstractAnsatz, 
                          config::VQEConfig; kwargs...)
...
function compare_vqe_convergence(results::Dict{String, VQEResult})
    ...
    for (method_name, result) in results
        ...
        convergence_iterations = findfirst(x -> abs(x - final_energy) < 1e-6, 
                                          reverse(result.energy_history))
        convergence_iterations = convergence_iterations === nothing ? 
