    ...
    function MolecularVQEAnalyzer(
        ...
        if use_test_hamiltonian
            ...
            molecular_system = create_molecular_hamiltonian(
                molecule; geometry=geometry, basis=basis, active_space=active_space
            )
...
"""
function run_standard_vqe(
    analyzer::MolecularVQEAnalyzer; num_iters::Int=300, verbose::Bool=true
)
    if verbose
    ...
    try
        ...
        # Run optimization
        result = run_vqe(
            vqe, analyzer.hamiltonian, initial_guess, num_iters; verbose=verbose
        )
...
function run_local_global_vqe(
    ...
    try
        ...
        results = run_vqe(
            vqe,
            analyzer.local_hamiltonian,
            analyzer.hamiltonian,
            initial_guess,
            num_iters,
            shift_iter;
            verbose=verbose,
...
function run_pretrained_vqe(
    ...
    try
        ...
        # Run optimization
        results = run_vqe(
            vqe, analyzer.hamiltonian, iters_vqe, iters_train; verbose=verbose
        )
...
"""
function compute_bp_diagnostics(
    analyzer::MolecularVQEAnalyzer, method_result::Dict; num_samples::Int=30
)
    if get(method_result, "fallback", false)
        # Return minimal diagnostics for fallback results
        return BarrenPlateauDiagnostics(
            1e-3, 1e-2, 1e-3, 1e-2, 1e-3, 0.0, [1e-2], [1.0], [1e-3]
        )
    end
    ...
    landscape_stats = compute_local_landscape_statistics(
        cost_function, final_params; num_samples=num_samples
    )
...
"""
function compute_local_landscape_statistics(
    cost_func,
...
function run_complete_analysis(
    ...
    for method in methods_to_run
        ...
        try
            if method == "standard"
                method_result = run_standard_vqe(
                    analyzer; num_iters=num_iters, verbose=verbose
                )
            elseif method == "local_global"
                method_result = run_local_global_vqe(
                    analyzer; num_iters=num_iters, verbose=verbose
                )
            elseif method == "adiabatic"
                method_result = run_adiabatic_vqe(
                    analyzer; num_iters=num_iters, verbose=verbose
                )
            elseif method == "sea"
                method_result = run_sea_vqe(analyzer; num_iters=num_iters, verbose=verbose)
            elseif method == "pretrained"
                method_result = run_pretrained_vqe(
                    analyzer; num_iters=num_iters, verbose=verbose
                )
            else
