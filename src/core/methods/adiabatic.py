...
global Hamiltonian, H(s) = (1-s) H_local + s H_global. qubap ramps the mixing
...
iteration and recovers qubap's per-iteration ramp as a limit. The method is also
optimizer-agnostic (any PennyLane optimizer), where qubap is SPSA-only. The
variational adiabatic schedule follows [2]. Like local_global, the local anchor is
the ground-state local cost [3], not qubap's global2local.
...
References:
    ...
    [2] Harwood, Trenev, Stober, Barkoutsos, Gujarati, Mostame, Greenberg,
        ...
        doi:10.1145/3479197.
    [3] Cerezo, Sone, Volkoff, Cincio, Coles, "Cost function dependent barren
...
from .local_global import local_cost_observable


def adiabatic(system: MolecularSystem, config: MethodConfig) -> MethodResult:
    """Adiabatic VQE: a staged anneal from H_local to H_global, warm-started.
    ...
    across the stages (so the total matches every other method):
        ...
        s_i = i / (n_stages - 1),   H(s_i) = (1 - s_i) H_local + s_i H_global
    ...
    The local anchor is the ground-state local cost, which shares the molecular
    ground state and so gives a sound anneal start. The max_iters iterations are
    spread across the stages so they sum to exactly max_iters (the first
    max_iters % n_stages stages get one extra), keeping the budget equal to every
    other method.
    """
    n = system.n_qubits
    ...
    h_local = local_cost_observable(system.ground_state, n)  # local anchor [3]
