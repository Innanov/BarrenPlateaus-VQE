...
function run_vqe(
    ...
    if verbose
        ...
        println(
            "  Total iterations: $(local_result.num_iterations + global_result.num_iterations)",
        )
        println(
            "  Total time: $(round(local_result.execution_time + global_result.execution_time, digits=2))s",
        )
    end
...
function run_vqe_combined(
    ...
    # Run the two-stage optimization
    results = run_vqe(
        vqe, hamiltonian_in, hamiltonian_out, initial_guess, max_iter, shift_iter; kwargs...
    )
...
function analyze_local_global_transition(results::Dict{String,Dict})
    ...
    local_improvement =
        length(local_energies) > 1 ? local_energies[1] - local_energies[end] : 0.0
    global_improvement =
        length(global_energies) > 1 ? global_energies[1] - global_energies[end] : 0.0
...
export LocalGlobalVQE,
    run_vqe, run_vqe_combined, create_local_global_vqe, run_local_global_vqe
