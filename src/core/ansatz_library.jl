...
struct EfficientSU2Ansatz <: AbstractAnsatz
    ...
    function EfficientSU2Ansatz(
        n_qubits::Int,
        n_layers::Int=1;
        rotation_gates::Vector{String}=["ry", "rz"],
        entanglement::String="circular",
    )
        circuit, n_params = build_efficient_su2(
...
"""
function build_efficient_su2(
    n_qubits::Int, n_layers::Int, rotation_gates::Vector{String}, entanglement::String
)
    if n_qubits < 1
...
struct StateEfficientAnsatz <: AbstractAnsatz
    ...
    function StateEfficientAnsatz(
        n_qubits::Int, n_layers::Int=1; depth_per_layer::Vector{Int}=Int[]
    )
...
struct CustomAnsatz <: AbstractAnsatz
    ...
    function CustomAnsatz(
        n_qubits::Int, circuit::AbstractBlock, description::String="Custom ansatz"
    )
...
function create_ansatz(ansatz_type::String, n_qubits::Int; kwargs...)
    ...
    elseif ansatz_type == "sea" || ansatz_type == "StateEfficient"
        ...
        return StateEfficientAnsatz(n_qubits, n_layers; depth_per_layer=depth_per_layer)
...
function generate_initial_parameters(
    ansatz::AbstractAnsatz; random_seed::Int=42, scale::Float64=0.1
)
...
function apply_ansatz(
    ...
    if length(parameters) != ansatz.n_parameters
        throw(
            ArgumentError(
                "Parameter count mismatch: expected $(ansatz.n_parameters), got $(length(parameters))",
            ),
        )
    end
...
"""
function optimize_ansatz_depth(
    ansatz_type::String,
    n_qubits::Int,
    hamiltonian::AbstractBlock;
    max_layers::Int=5,
...
"""
function compare_ansatz_expressivity(
    ansatz_list::Vector{AbstractAnsatz}, hamiltonian::AbstractBlock; num_trials::Int=20
)
